...
$canStartPublicLive = $canStartPublicLive ?? \community\videolibrary\components\AccessPolicy::canStartPublicLive();
$publicCategoryOptions = $publicCategoryOptions ?? \community\videolibrary\models\PublicLiveCategory::options();
?>
...
<?= Html::a('Vorhandene Livequellen im Video-Dienst prüfen und entfernen', $liveManagementUrl, ['target' => '_blank', 'rel' => 'noopener noreferrer', 'class' => 'btn btn-default']) ?>
<p>Melde dich dort mit dem technischen Videokonto an, das in der Mediathek eingerichtet ist.
...
Controller <?= Html::encode(defined('community\\videolibrary\\controllers\\LiveController::DIAGNOSTICS_VERSION') ? \community\videolibrary\controllers\LiveController::DIAGNOSTICS_VERSION : 'vor 2.8.9 / unbekannt') ?> ·
Fehlerbehandlung <?= Html::encode(defined('community\\videolibrary\\components\\LiveError::VERSION') ? \community\videolibrary\components\LiveError::VERSION : 'vor 2.8.9 / unbekannt') ?>
</small></p>
...
<?php if (!empty($canStartPublicLive)): ?>
<?= $form->field($model,'publicEvent')->checkbox()->label('Öffentliches Event über den Live-Kanal ankündigen')->hint('Das Event erscheint auf der öffentlichen Terminliste. Der gemeinsame Kanal zeigt bis zum Beginn sein festes Wartebild.') ?>
<div id="pt-public-live-categories" style="display:none">
<?= $form->field($model, 'publicCategories')->checkboxList($publicCategoryOptions ?? [])->label('Kategorien') ?>
<?= $form->field($model, 'newPublicCategories')->textInput(['maxlength' => 500])->label('Neue Kategorien')->hint('Mehrere neue Kategorien mit Komma trennen. Sie werden als öffentliche Playlisten angelegt.') ?>
</div>
<?php endif; ?>
...
<?php $this->registerJs("(function(){var c=document.getElementById('liveform-schedule'),f=document.getElementById('pt-live-schedule-fields'),p=document.getElementById('liveform-publicevent'),pc=document.getElementById('pt-public-live-categories');if(!c||!f)return;function t(){f.style.display=c.checked?'block':'none';if(pc)pc.style.display=p&&p.checked&&c.checked?'block':'none'}c.addEventListener('change',t);if(p)p.addEventListener('change',t);t();})();"); ?>
